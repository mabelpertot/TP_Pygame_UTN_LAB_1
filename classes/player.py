...
class Player: #Representa al jugador en el juego.
    ...
    def move_left(self): #Mueve al jugador hacia la izquierda.
        if self.rect.left > 0: 
            self.rect.x -= self.speed
            self.direction = 'left'
            self.image = pygame.transform.flip(self.original_image, True, False)

    def move_right(self): #Mueve al jugador hacia la derecha.
        if self.rect.right < WIDTH:
            self.rect.x += self.speed
            self.direction = 'right'
            self.image = self.original_image

    def move_up(self): #Mueve al jugador hacia arriba.
        if self.rect.top > 0:
            self.rect.y -= self.speed
            self.direction = 'up'

    def move_down(self): #Mueve al jugador hacia arriba.
